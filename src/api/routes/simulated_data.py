...
# Load simulated data from Supabase
...
async def visualize_real_data(
  ...
  '''
  Function to visualize simulated data from the IoT network.
  '''
  ...
  try:
    while offset < steps:
      ...
      response = supabase.table("iot_simulation").select(f'{safe_category}, timestamp') \
